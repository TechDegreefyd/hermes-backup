...
with open('clean_css.txt','r', errors='ignore') as f: clean_css = f.read()
clean_css = re.sub(r'#cb-[^}]+\{[^}]+\}', '', clean_css)
with open('clean_js.txt','r', errors='ignore') as f: clean_js = f.read()
...
payload = {"to": WHATSAPP_GROUP, "media": f"data:text/html;name=Degreefyd_Final_Master_White.html;base64,{b64}", "caption": f"🏆 **EXECUTIVE DASHBOARD - MAY 4TH (LATEST DATA)**\n\n✅ **Accuracy:** Calculated directly from 'Day Wise CAC' raw logs.\n✅ **MTD May:** Matches the expected ₹81,238.69.\n✅ **Dynamic Trends:** Graphs now automatically include data till {today.strftime('%d %b')}.\n✅ **Fixes:** Restored Graph Comparison and Safari drilldown toggles."}
