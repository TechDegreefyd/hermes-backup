...
for plat in sorted(platforms, reverse=True): 
    ...
    for acct in accounts:
        ...
        <details class="acc-card">
            ...
            <div class="table-wrap">
                <table>
                    <thead>
                        <tr>
                            <th class="text-left sticky-col">Platform</th>
        ...
        for camp in campaigns:
            ...
            for _, row in camp_df.iterrows():
                ...
                html_content += f'''
                    <tr>
                        <td class="text-left sticky-col">{plat}</td>
                        <td class="text-left">{acct}</td>
                        <td class="text-left">{row.get('Date', '-')}</td>
                        <td class="text-left" style="color: #cbd5e1; max-width:250px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="{camp_name}">{camp_name}</td>
                        <td class="text-left" style="color: #94a3b8; max-width:250px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="{row.get('Ad Name', '-')}">{row.get('Ad Name', '-')}</td>
...
<title>Degreefyd Final Export</title>
<style>
...
<div class="wrap">
  <div class="hdr">
    <h1>Degreefyd Raw Export</h1>
    <p>Platform ➔ Account ➔ Date ➔ Campaign ➔ Ad Name</p>
  </div>
  {html_content}
</div>
</body>
...
file_path = "/workspace/Degreefyd_Raw_Export_Correct.html"
...
payload = {
    "to": WHATSAPP_GROUP,
    "media": f"data:text/html;name=Degreefyd_Raw_Export.html;base64,{b64}",
    "caption": "👑 **Degreefyd Raw Export (Perfect Match)**\n\nI have matched the *exact* format you pasted in the chat:\n`Platform | Account | Date | Campaign | Ad Name`.\n\n✅ **One Section for Meta, One for Google:** As requested.\n✅ **Accounts are Clickable:** When you open an account, you get the exact flat raw rows underneath it showing Platform, Account, Date, Campaign, Ad Name, and all calculated metrics in a single line without any confusing nested campaign totals breaking it up.\n✅ **Clean UI:** No graphs. Just exactly what you pasted."
}
