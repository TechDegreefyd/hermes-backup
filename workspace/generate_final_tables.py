...
ranges = [
    "'Campaign Wise - YTD'!A1:S200",
    "'Campaign Wise - MTD'!A1:S200",
    "'Campaign Wise - FTD'!A1:S200",
    "DSA_graph1!A1:F100",
    "DSA_graph2!A1:C100",
...
def make_clean_table(headers, rows):
    ...
    for row in rows:
        ...
        for i, cell in enumerate(row):
            ...
            is_num = str(val).replace('.','').replace(',','').replace('%','').replace('-','').isdigit() or str(val).startswith('₹')
            css_class = "num" if is_num else ""
            if "%" in str(val) and pnum(val) > 20: css_class += " warn"
...
def get_campaign_rows(sheet_name):
    ...
    out = []
    for r in raw_rows[1:]:
        if not r or (len(r) > 2 and not str(r[0]).strip() and not str(r[1]).strip() and not str(r[2]).strip()): continue
        def get_val(idx): return r[idx] if len(r) > idx else "0"
        
        camp_name = get_val(2)
        if not camp_name: camp_name = get_val(1)
        ...
        row_data = [
            ...
            get_val(8),                      
            f"₹{pnum(get_val(9)):,.0f}",     
            f"₹{pnum(get_val(10)):,.0f}",    
            get_val(11) if "₹" in str(get_val(11)) else f"₹{pnum(get_val(11)):,.0f}", 
            get_val(12) if "₹" in str(get_val(12)) else f"₹{pnum(get_val(12)):,.0f}", 
            get_val(13),                     
            get_val(14),                     
            get_val(15),                     
            get_val(16),                     
...
full_headers = ["Campaign", "Spends", "Panel Leads", "LMS Leads", "Dup %", "FFH", "ADM", "Inv_Var", "CPL Panel", "CPL LMS", "CAC FFH", "CAC ADM", "ARPU", "CAC/ARPU", "L2F", "L2A", "F2A"]

sections = {}
sections['summary'] = f"""
<h2 class="sec-title">Year-To-Date (YTD) Full Data</h2>
{make_clean_table(full_headers, get_campaign_rows("Campaign Wise - YTD"))}
<h2 class="sec-title">Month-To-Date (MTD) Full Data</h2>
{make_clean_table(full_headers, get_campaign_rows("Campaign Wise - MTD"))}
<h2 class="sec-title">Fortnight-To-Date (FTD) Full Data</h2>
{make_clean_table(full_headers, get_campaign_rows("Campaign Wise - FTD"))}
"""

def get_daily_rows(cpl_sheet, lead_sheet):
    cpl_rows = data.get(cpl_sheet, [])
    lead_rows = data.get(lead_sheet, [])
    df, x_col, cpl_p, cpl_l, lead_p, lead_l = prep_graph_data(cpl_rows, lead_rows)
    if df.empty: return []
    out = []
    for _, row in df.iterrows():
        l_p = pnum(row[lead_p])
        l_l = pnum(row[lead_l])
        dup_pct = ((l_p - l_l) / l_p * 100) if l_p > 0 else 0
        out.append([
            row[x_col],
            f"{l_p:g}",
            f"{l_l:g}",
...
daily_headers = ["Date", "Panel Leads", "LMS Leads", "Dup %", "CPL Panel", "CPL LMS"]

try:
    df_dsa, x_dsa, cpl_p_dsa, cpl_l_dsa, lead_p_dsa, lead_l_dsa = prep_graph_data(data.get("DSA_graph1", []), data.get("DSA_graph2", []))
    sections['dsa'] = f"""
    <div class="card">{create_dual_chart(df_dsa, x_dsa, lead_p_dsa, lead_l_dsa, "DSA Leads", "Panel Leads", "LMS Leads", "#22d98a", "#a78bfa")}</div>
    <div class="card">{create_dual_chart(df_dsa, x_dsa, cpl_p_dsa, cpl_l_dsa, "DSA CPL", "CPL Panel", "CPL LMS", "#f5a623", "#ff4d6d")}</div>
    <h2 class="sec-title">DSA Daily Raw Data</h2>
    {make_clean_table(daily_headers, get_daily_rows("DSA_graph1", "DSA_graph2"))}
    """
...
try:
    ...
    sections['brand'] = f"""
    <div class="card">{create_dual_chart(df_brand, x_brand, lead_p_brand, lead_l_brand, "Brand Leads", "Panel Leads", "LMS Leads", "#22d98a", "#a78bfa")}</div>
    <div class="card">{create_dual_chart(df_brand, x_brand, cpl_p_brand, cpl_l_brand, "Brand CPL", "CPL Panel", "CPL LMS", "#f5a623", "#ff4d6d")}</div>
    <h2 class="sec-title">Brand Daily Raw Data</h2>
    {make_clean_table(daily_headers, get_daily_rows("Brand graph1", "Brand graph2"))}
    """
...
try:
    ...
    sections['meta'] = f"""
    <div class="card">{create_dual_chart(df_meta, x_meta, lead_p_meta, lead_l_meta, "Meta Leads", "Panel Leads", "LMS Leads", "#22d98a", "#a78bfa")}</div>
    <div class="card">{create_dual_chart(df_meta, x_meta, cpl_p_meta, cpl_l_meta, "Meta CPL", "CPL Panel", "CPL LMS", "#f5a623", "#ff4d6d")}</div>
    <h2 class="sec-title">Meta Daily Raw Data</h2>
    {make_clean_table(daily_headers, get_daily_rows("graph1", "graph2"))}
    """
...
th {{ background: #1e293b; color: #cbd5e1; font-size: 11px; text-transform: uppercase; font-weight: 800; padding: 12px 14px; border-bottom: 1px solid #334155; white-space: nowrap; }}
td {{ padding: 12px 14px; border-bottom: 1px solid #1e293b; font-size: 12px; color: #f8fafc; white-space: nowrap; }}
...
<div class="wrap">
  <div class="hdr">
    ...
    <p>Comprehensive Campaign Analysis & Leads Duplication Tracker</p>
  </div>
  <div class="tabs">
...
payload = {
    ...
    "caption": "📊 **Degreefyd Dashboard (Complete Columns)**\n\n✅ Added **Campaign Name**.\n✅ Added **Duplication %** formula `[(Panel - LMS) / Panel] * 100`.\n✅ Included all requested columns: **FFH, ADM, Inv_Var, CAC_FFH, CAC_Adm, ARPU, L2F, L2A, F2A**.\n✅ High-res charts without overlapping text."
}
