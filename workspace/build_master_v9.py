...
def build_summary_table(raw_rows):
    ...
    # We rebuild the platform summary by isolating the rows that have "Total" in the Platform column
    # or "Grand Total"
    rows = []
    ...
    for r in raw_rows[1:]:
        ...
        if "Total" in str(r[0]):
            ...
            if sp > 0:
                dup = pnum(r[17]) if len(r)>17 else 0 # Dup is not natively in the sheet? No, wait.
                # In Campaign Wise, columns are:
                # 0:Platform, 1:Account, 2:Campaign, 3:Spends, 4:Pannel_Leads, 5:Leads_LMS, 6:FFH, 7:ADM, 8:Inv_Var, 
                # 9:CPL_Pannel, 10:CPL_LMS, 11:CAC_FFH, 12:CAC_Adm, 13:ARPU, 14:CAC/ARPU, 15:L2F, 16:L2A, 17:F2A
                # Let's calculate Dup:
    ...
    def r_tr(label, r, custom_bg=None, is_total=False):
        ...
        # Re-formatting numbers to keep them clean
        return f"""<tr{style}>
            <td class="text-left sticky-col"{style}><strong>{label}</strong></td>
            <td class="num">{format_currency_full(pnum(v(3)))}</td>
            <td class="num">{format_num(pnum(v(4)))}</td>
            <td class="num">{format_num(pnum(v(5)))}</td>
            <td class="num"{gcs('dup', dup)}>{format_pct(dup)}</td>
            <td class="num">{format_num(pnum(v(6)))}</td>
            <td class="num">{format_num(pnum(v(7)))}</td>
            <td class="num">{format_currency_full(pnum(v(8)))}</td>
            <td class="num"{gcs('cpl_p', cpl_p)}>{format_currency_full(cpl_p)}</td>
            <td class="num"{gcs('cpl_l', cpl_l)}>{format_currency_full(cpl_l)}</td>
            <td class="num"{gcs('cac_f', cac_f)}>{format_currency_full(cac_f)}</td>
            <td class="num"{gcs('cac_a', cac_a)}>{format_currency_full(cac_a)}</td>
            <td class="num">{format_currency_full(pnum(v(13)))}</td>
            <td class="num">{v(14)}</td>
            <td class="num"{gcs('l2f', l2f)}>{format_pct(l2f)}</td>
            <td class="num"{gcs('l2a', l2a)}>{format_pct(l2a)}</td>
            <td class="num"{gcs('f2a', f2a)}>{format_pct(f2a)}</td>
        </tr>"""
    ...
    if grand_total:
        html += r_tr("OVERALL TOTAL", grand_total, custom_bg="#f8fafc", is_total=True)
        
    return html + '</tbody></table></div>'

def build_kpi(label, date, r, timeframe):
    # r is the grand total row
    def v(idx): return r[idx] if len(r) > idx else "-"
    
    # We only build ONE kpi strip (the scrollable one)
...
def build_h_table(h, pref):
    ...
    for plat, accs in h.items():
        ...
        for acct, data in accs.items():
            ...
            # Default state: NO CHECKED ATTRIBUTE = OFF BY DEFAULT
            ...
            html += f"""<tbody><tr class="account-row">
                ...
                <td class="num">{format_num(pnum(v(4)))}</td>
                <td class="num">{format_num(pnum(v(5)))}</td>
                <td class="num">{format_pct(dup)}</td>
                <td class="num">{format_num(pnum(v(6)))}</td>
                <td class="num">{format_num(pnum(v(7)))}</td>
                <td class="num">{format_currency_full(pnum(v(8)))}</td>
                <td class="num">{format_currency_full(pnum(v(9)))}</td>
                <td class="num">{format_currency_full(pnum(v(10)))}</td>
                <td class="num">{format_currency_full(pnum(v(11)))}</td>
                <td class="num">{format_currency_full(pnum(v(12)))}</td>
                <td class="num">{format_currency_full(pnum(v(13)))}</td>
                <td class="num">{v(14)}</td>
                <td class="num">{format_pct(pnum(v(15)))}</td>
                <td class="num">{format_pct(pnum(v(16)))}</td>
                <td class="num">{format_pct(pnum(v(17)))}</td>
            ...
            for c in data['campaigns']:
                ...
                html += f"""<tr class="camp-row">
                    <td class="text-left sticky-col"><span style="padding-left:25px;">\u21b3 {cv(2)}</span></td>
                    <td class="num">{format_currency_full(pnum(cv(3)))}</td>
                    <td class="num">{format_num(pnum(cv(4)))}</td>
                    <td class="num">{format_num(pnum(cv(5)))}</td>
                    <td class="num">{format_pct(cdup)}</td>
                    <td class="num">{format_num(pnum(cv(6)))}</td>
                    <td class="num">{format_num(pnum(cv(7)))}</td>
                    <td class="num">{format_currency_full(pnum(cv(8)))}</td>
                    <td class="num">{format_currency_full(pnum(cv(9)))}</td>
                    <td class="num">{format_currency_full(pnum(cv(10)))}</td>
                    <td class="num">{format_currency_full(pnum(cv(11)))}</td>
                    <td class="num">{format_currency_full(pnum(cv(12)))}</td>
                    <td class="num">{format_currency_full(pnum(cv(13)))}</td>
                    <td class="num">{cv(14)}</td>
                    <td class="num">{format_pct(pnum(cv(15)))}</td>
                    <td class="num">{format_pct(pnum(cv(16)))}</td>
                    <td class="num">{format_pct(pnum(cv(17)))}</td>
                </tr>"""
            html += '</tbody>'
...
with open('latest_cac.json', 'r') as f: 
    # MTD/FTD are likely in Campaign Wise - MTD / FTD as well. Let's fetch them to be accurate.
    pass
...
fetch_sheet("'Campaign Wise - MTD'!A1:S1000", "mtd_sheet.json")
fetch_sheet("'Campaign Wise - FTD'!A1:S1000", "ftd_sheet.json")
...
# Assembling P1 Panel
# requested structure: 
# 1. First the overall summary of FTD, MTD, YTD
# 2. Then deep drilldown
# 3. No "AI Intelligence" block
# 4. No double gradient KPI hero, just the simple KPI band
p1 = f"""
