...
def rats(v):
    sp = v.get('Spends', 0); lp = v.get('Pannel_Lead', 0); ll = v.get('Lead_LMS', 0)
    ff = v.get('FFH', 0); ad = v.get('Adm', 0); iv = v.get('Invoicing_Var', 0)
    cpl_p = sp/lp if lp > 0 else 0; cpl_l = sp/ll if ll > 0 else 0
    cac_f = sp/ff if ff > 0 else 0; cac_a = sp/ad if ad > 0 else 0
    arpu = iv/ad if ad > 0 else 0; cac_arpu = (cac_a / arpu) if arpu > 0 else 0
    l2f = (ff / ll * 100) if ll > 0 else 0; l2a = (ad / ll * 100) if ll > 0 else 0
    f2a = (ad / ff * 100) if ff > 0 else 0; dup = ((lp - ll) / lp * 100) if lp > 0 else 0
    return {'cpl_p': cpl_p, 'cpl_l': cpl_l, 'cac_f': cac_f, 'cac_a': cac_a, 'arpu': arpu, 'cac_arpu': cac_arpu, 'l2f': l2f, 'l2a': l2a, 'f2a': f2a, 'dup': dup}
...
# --- DATA PROCESSING ---
with open('latest_cac.json', 'r') as f: raw_data = json.load(f)
...
def render_tr(label, data, is_account=False, is_camp=False, is_daily=False, cid=None, pi="", indent=0, custom_bg=None, custom_co=None, overall_data=None):
    ...
    return f"""<tr class="{cls}"{style}><td class="text-left sticky-col"{td_style}>{lbl_content}</td><td class="num">{format_currency_full(s['Spends'])}</td><td class="num">{format_num(s['Pannel_Lead'])}</td><td class="num">{format_num(s['Lead_LMS'])}</td><td class="num"{gcs('dup', r['dup'])}>{format_pct(r['dup'])}</td><td class="num">{format_num(s['FFH'])}</td><td class="num">{format_num(s['Adm'])}</td><td class="num">{format_currency_full(s['Invoicing_Var'])}</td><td class="num"{gcs('cpl_p', r['cpl_p'])}>{format_currency_full(r['cpl_p'])}</td><td class="num"{gcs('cpl_l', r['cpl_l'])}>{format_currency_full(r['cpl_l'])}</td><td class="num"{gcs('cac_f', r['cac_f'])}>{format_currency_full(r['cac_f'])}</td><td class="num"{gcs('cac_a', r['cac_a'])}>{format_currency_full(r['cac_a'])}</td><td class="num">{format_currency_full(r['arpu'])}</td><td class="num">{format_pct(r['cac_arpu']*100)}</td><td class="num"{gcs('l2f', r['l2f'])}>{format_pct(r['l2f'])}</td><td class="num"{gcs('l2a', r['l2a'])}>{format_pct(r['l2a'])}</td><td class="num"{gcs('f2a', r['f2a'])}>{format_pct(r['f2a'])}</td></tr>"""
...
def build_kpi(period_label, date_label, stats, timeframe_label):
    r = rats(stats)
...
def create_line_pair(g_file, plat):
    ...
    dg = pd.DataFrame(data[1:], columns=[c.strip() for c in data[0]]); x = dg.columns[0]
    def fc(d, p):
        for c in d.columns:
            if p.upper() in str(c).upper().replace(' ', '').replace('_', ''): return c
        return None
    sp, lp, ll = fc(dg, 'SPEND'), fc(dg, 'PANNEL'), fc(dg, 'LMS')
    for c in [sp, lp, ll]: 
        if c: dg[c] = dg[c].apply(pnum)
    dg['Date_Parsed'] = pd.to_datetime(dg[x], errors='coerce')
    dg = dg.sort_values('Date_Parsed').dropna(subset=['Date_Parsed'])
    dg['X'] = dg['Date_Parsed'].dt.strftime('%b %d')
    dg['CPL_P'] = dg[sp]/dg[lp]; dg['CPL_L'] = dg[sp]/dg[ll]; dg.replace([np.inf, -np.inf], 0, inplace=True)
    def chart(y1, y2, l1, l2, t, is_cpl):
        ...
        ax.plot(dg['X'], dg[y1], marker='o', color='#059669' if not is_cpl else '#d97706', label=l1, linewidth=2.5, markersize=6)
        ax.plot(dg['X'], dg[y2], marker='s', color='#7c3aed' if not is_cpl else '#dc2626', label=l2, linewidth=2.5, markersize=6)
        for i, v in enumerate(dg[y1]): ax.text(i, v, (f"₹{v:,.0f}" if is_cpl else f"{v:,.0f}"), ha='center', va='bottom', fontweight='bold', fontsize=9, bbox=dict(facecolor='white', alpha=0.6, edgecolor='none', pad=1))
        for i, v in enumerate(dg[y2]): ax.text(i, v, (f"₹{v:,.0f}" if is_cpl else f"{v:,.0f}"), ha='center', va='top', fontweight='bold', fontsize=9, bbox=dict(facecolor='white', alpha=0.6, edgecolor='none', pad=1))
...
# --- ASSEMBLE ---
with open('clean_css.txt', 'r') as f: css = f.read()
with open('clean_js.txt', 'r') as f: js = f.read()
with open('clean_p5_html.txt', 'r') as f: p5_html = f.read()
...
dl, dc = create_line_pair('latest_dsa_g1.json', 'DSA'); bl, bc = create_line_pair('latest_brand_g1.json', 'Brand'); ml, mc = create_line_pair('latest_meta_g1.json', 'META ADS')
...
payload = {"to": WHATSAPP_GROUP, "media": f"data:text/html;name=Degreefyd_Final_Master_White.html;base64,{b64}", "caption": "🏆 **EXECUTIVE DASHBOARD - FULLY REBUILT**\n\n✅ **Clean Build:** Removed all duplicated headers and visual bugs.\n✅ **Fixed Emojis:** Icons in tabs now render correctly.\n✅ **Latest Data:** Updated with newest results from the sheet.\n✅ **Tab 5 Builder:** Preserved the interactive chart builder logic."}
