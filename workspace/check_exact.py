...
# Exact check!
ftd_df = df_full[df_full['Date_Parsed'] == today]
print('--- MAY 4TH FTD CHECK ---')
print('Adm sum:', ftd_df['Adm'].sum())
print('Inv sum:', ftd_df['Invoicing_Var'].sum())
...
google_df = ftd_df[ftd_df['Platform'].str.contains('Google', case=False, na=False)]
meta_df = ftd_df[ftd_df['Platform'].str.contains('Meta', case=False, na=False)]
g_stats = aggregate_df(google_df)
m_stats = aggregate_df(meta_df)
ov_stats = aggregate_df(ftd_df)

print('g_stats ADM:', g_stats[7])
print('m_stats ADM:', m_stats[7])
print('ov_stats ADM:', ov_stats[7])
