...
OUTPUT_PATH = "/workspace/Degreefyd_Regular_Master_White.html"
...
df['same_day_pipe'] = False   # marks rows that are same-day closures (should show in FTD)
...
# In Regular sheet, Campaign ID maps to Ad Name. Fallback to Campaign Name.
c_id = df_ffh.get('Campaign ID', pd.Series(['']*len(df_ffh))).astype(str).str.strip()
c_name = df_ffh.get('Campaign Name', pd.Series(['']*len(df_ffh))).astype(str).str.strip()
df_ffh['Camp_Name'] = np.where(c_id != '', c_id, c_name)
...
#             These rows land on Form Date → Appear in FTD.
#   Step 3 — inject old-pipeline Adm+Inv by Adm Date (Form Date < Adm Date)
#             These rows land on Adm Date → Outside FTD.
#
# To ensure FTD Inv/Adm correctly shows only same-day, we must:
# 1. Mark rows injected in Step 2 as 'same_day_pipe=True'
# 2. Filter FTD to only include 'same_day_pipe=True' rows (or FFH-only rows)

# Step 1 — FFH by Form Date
for _, r in ffh_grp.iterrows():
    inject(r['Form_Date'], r['Camp_Name'], float(r['FFH_Count']), 0.0, 0.0)

# Step 2 — same-day: Form Date == Adm Date → inject on Form Date, set same_day_pipe=True
...
for _, r in same_day_grp.iterrows():
    # Helper to tag same-day rows for FTD inclusion
    mask_d = df['Date_Parsed'] == r['Form_Date']
    mask_c = (df['Ad Name'].str.strip() == r['Camp_Name']) | (df['Campaign'].str.strip() == r['Camp_Name'])
    hits = df.index[mask_d & mask_c]
    if len(hits) > 0:
        idx = hits[0]
        df.at[idx, 'Adm']           += float(r['Adm_Count'])
        df.at[idx, 'Invoicing_Var'] += float(r['Inv_Sum'])
        df.at[idx, 'same_day_pipe'] = True
    else:
        # Should not happen as FFH injection Step 1 already created the row
...
# ── VERIFICATION PRINT ────────────────────────────────────────────────────────
ftd_df = df[(df['Date_Parsed'].dt.date == today.date()) & ((~df['pipeline_only']) | (df['same_day_pipe']))]
...
<div class="wrap">
  <div class="hdr">
    <h1>Degreefyd Master Dashboard (Regular)</h1>
...
caption = (f"📊 *DEGREEFYD REGULAR DASHBOARD — {today.strftime('%d %b %Y').upper()}* (v10)\n\n"
    ...
    try:
        resp = requests.post(
            ...
            json={"to": WHATSAPP_GROUP,
                  "media": f"data:text/html;name=Degreefyd_Regular_Master_{today.strftime('%d%b%Y')}.html;base64,{b64}",
                  "caption": caption},
            timeout=60
        )
