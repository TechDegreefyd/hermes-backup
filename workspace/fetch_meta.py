...
def get_insights(account_id):
    ...
    while url:
        ...
        if 'error' in response:
            print(f"Error fetching act_{account_id}: {response['error']['message']}")
            break
...
if full_results:
    ...
    with open('meta_insights_raw.csv', 'w', newline='') as f:
        dict_writer = csv.DictWriter(f, fieldnames=keys)
        dict_writer.writeheader()
        dict_writer.writerows(full_results)
    print(f"Saved {len(full_results)} records to meta_insights_raw.csv")
else:
    print("No data found.")
